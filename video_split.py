...
class VideoSplit:
    ...
    def generate_tag_based(self, n=4):
        vectorizer = TfidfVectorizer(
            input="filename", ngram_range=(1, 1), token_pattern=r"(?u)\b[\w\d]+\b"
        )
        ...
        for i in range(n):
            result[f"list_{i+1}"] = []
        for i in range(0, 4):
            cluster_list[f"cluster_{i}"] = np.array(self.all_vidieo_name)[
                kmean.labels_ == i
            ].tolist()
        ...
        for key in cluster_list.keys():
            ...
            for list_key in random_video_path_id.keys():
                result[list_key] += np.array(cluster_list[key])[
                    random_video_path_id[list_key]
                ].tolist()
        return result
    ...
    def generate_random(num_vidieos, n=4):
        ...
        for i in range(n):
            result[f"list_{i+1}"] = final_shuffle[
                math.ceil(len(final_shuffle) / n)
                * i : math.ceil(len(final_shuffle) / n)
                * (i + 1)
            ]
        return result
