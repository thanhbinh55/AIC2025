...
def merge_searching_results(list_scores, list_indices, list_image_paths):
    ...
    return (
        normalized_list_scores[unique_indices],
        list_indices_refined[unique_indices],
        list_image_paths_refined[unique_indices],
    )
...
def merge_searching_results_by_addition(list_scores, list_indices):
    ...
    for scores in list_scores:
        new_scores = (scores - np.min(scores)) / (
            np.max(scores) - np.min(scores) + 0.000001
        )
