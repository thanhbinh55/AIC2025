...
class speech_retrieval(semantic_extract, load_file):
    def __init__(
        ...
        context_path=os.path.join(PROJECT_ROOT, "dict/audio_ASR"),
        ...
        if enable_semantic:
            ...
            not os.path.exists(
                os.path.join(context_vector_path, f"tfidf_transform_speech.pkl")
            )
        ) or test_mode:
        ...
        with open(
            os.path.join(context_vector_path, "tfidf_transform_speech.pkl"), "rb"
        ) as f:
            self.tfidf_transform = pickle.load(f)
        self.context_matrix = scipy.sparse.load_npz(
            os.path.join(context_vector_path, f"sparse_context_matrix_speech.npz")
        )
