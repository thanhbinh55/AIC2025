...
class MyFaiss:
    def __init__(
        ...
        self.index_nomic = self.load_bin_file(bin_nomic_file)
        ...
        self.id2img = self.load_json_file(json_path)
    ...
    def image_search(self, id_query, k):
        query_feats = self.index_nomic.reconstruct(id_query).reshape(1, -1)

        scores, idx_image = self.index_nomic.search(query_feats, k=k)
    ...
    def text_search(self, text, index, k, model_type):
        ...
        ###### SEARCHING #####
        if model_type == "nomic":
            index_choosed = self.index_nomic
        else:
        ...
        if index is None:
            ...
            scores, idx_image = index_choosed.search(
                text_features, k=k, params=faiss.SearchParametersIVF(sel=id_selector)
            )
        idx_image = idx_image.flatten()
    ...
    def asr_retrieval_helper(self, asr_input, k, index, semantic, keyword):
        ...
        if index is not None:
            ...
            asr_scores, asr_idx_image = self.asr_post_processing(
                tmp_asr_scores, tmp_asr_idx_image, k
            )
    ...
    def reranking(self, prev_result, lst_pos_vote_idxs, lst_neg_vote_idxs, k):
        ...
        query_feats = self.index_nomic.reconstruct_batch(lst_vote_idxs)
        lst_scores, lst_idx_images = self.index_nomic.search(
            query_feats,
            k=min(k, len(result)),
        ...
        infos_query = list(map(self.id2img.get, list(list_ids)))
