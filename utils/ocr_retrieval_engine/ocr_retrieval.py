...
class ocr_retrieval(load_file):
    def __init__(
        ...
        self.context_sparse_matrix_ocr = scipy.sparse.load_npz(
            os.path.join(
                os.path.join(PROJECT_ROOT, ocr_embed_path),
                f"sparse_context_matrix_ocr.npz",
            )
        )
    ...
    def get_tfidf_score(
        ...
        if index is None:
            ...
            scores = vectorize.dot(
                self.context_sparse_matrix_ocr[index, :].T
            ).toarray()[0]
            sort_index = np.argsort(scores)[::-1][:k]
            scores = scores[sort_index]
...
if __name__ == "__main__":
    obj = ocr_retrieval()
    score, index = obj(
        "Tính mạng nguy kịch vì dùng thuốc gia truyền trên mạng", 3
    )  # return index of top k result
    print(score)
    print(index)
