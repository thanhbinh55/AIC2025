...
class load_file:
    """
    This class is used to load the context data and create tf-idf vectorizer for each data type.
    It will save the tf-idf vectorizer and context matrix to the specified path.
    """
        ...
        for data_type in all_datatpye:
            if not os.path.exists(
                ...
                if context_data == None:
                    clean_data_paths = os.path.join(
                        PROJECT_ROOT, clean_data_path[data_type]
                    )
                ...
                tfidf_transform[data_type] = TfidfVectorizer(
                    input="content",
                    ngram_range=ngram_range,
                    token_pattern=r"(?u)\b[\w\d]+\b",
                )
                context_matrix[data_type] = (
                    tfidf_transform[data_type].fit_transform(context).tocsr()
                )
                print(tfidf_transform[data_type].get_feature_names_out()[:10])
                print(context_matrix[data_type].shape)
                with open(
                    os.path.join(
                        save_tfids_object_path, f"tfidf_transform_{data_type}.pkl"
                    ),
                    "wb",
                ) as f:
                ...
                scipy.sparse.save_npz(
                    os.path.join(
                        save_tfids_object_path, f"sparse_context_matrix_{data_type}.npz"
                    ),
                    context_matrix[data_type],
                )
    ...
    def load_context(self, clean_data_paths, input_datatype):
        ...
        if input_datatype == "txt":
            data_paths = []
            Lxx_data_paths = glob.glob(clean_data_paths)
            Lxx_data_paths.sort()
            for Lxx_data_path in Lxx_data_paths:
                data_path = glob.glob(Lxx_data_path + "/*.txt")
            ...
            context_paths.sort()
            for Lxx_context_path in context_paths:
                paths = glob.glob(Lxx_context_path + "/*.json")
                paths.sort(reverse=False, key=lambda x: int(x[-8:-5]))
                for path in paths:
                    with open(path) as f:
                        context += [
                            self.preprocess_text(" ".join(line))
...
class object_retrieval(load_file):
    def __init__(
        ...
        all_datatpye = [
            key for key in clean_data_path.keys() if (clean_data_path[key] != None)
        ]
        super().__init__(
        ...
        for data_type in all_datatpye:
            ...
            self.context_matrix[data_type] = scipy.sparse.load_npz(
                os.path.join(
                    save_tfids_object_path, f"sparse_context_matrix_{data_type}.npz"
                )
            )
    ...
    def __call__(
        ...
        for input_type in ["bbox", "color", "class", "tag", "number"]:
            if texts[input_type] is not None:
                scores_, idx_image_ = self.find_similar_score(
                    texts[input_type], input_type, k, index=index
                )
    ...
    def find_similar_score(
        ...
        if (
            ...
            scores = vectorize.dot(
                self.context_matrix[transform_type][index, :].T
            ).toarray()[0]
            sort_index = np.argsort(scores)[::-1][:k]
            scores = scores[sort_index]
