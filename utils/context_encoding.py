...
class VisualEncoding:
    ...
    def visualize_grid(self, grid_vis=None):
        ...
        for i in range(self.n_row * self.n_col):
            ...
            org = (
                int((x_start + (x_end - x_start) / 2) * vis_w),
                int((y_start + (y_end - y_start) / 2) * vis_h),
            )

            # Draw text
            grid_vis = cv2.putText(
                grid_vis, label, org, font, fontScale, color, thickness, cv2.LINE_AA
            )
            # Draw grid
            grid_vis = cv2.rectangle(
                grid_vis,
                (int(x_start * vis_w), int(y_start * vis_h)),
                (int(x_end * vis_w), int(y_end * vis_h)),
                color,
                thickness,
            )
    ...
    def encode_bboxes(self, bboxes, labels):
        ...
        for bbox_idx, loc_idx in zip(bboxes_idx, locs_idx):
            context.append(
                self.grid_labels[loc_idx]
                + self.classes[labels[bbox_idx]].replace(" ", "")
            )
    ...
    def encode_colors(self, bboxes, colors):
        ...
        for bbox_idx, loc_idx in zip(bboxes_idx, locs_idx):
            context.append(
                self.grid_labels[loc_idx] + colors[bbox_idx].replace("_", "")
            )
        context = " ".join(map(str, context))
        return context
