...
class semantic_extract:
    def __init__(
        ...
        context_vector_path=os.path.join(
            PROJECT_ROOT, "data/TransNetDatabase/CaptionFeatures/context_vector.npy"
        ),
        input_datatype="txt",
        ...
        if not os.path.exists(context_vector_path):
            self.raw_data = self.generate_context_embedding(
                context_path, context_vector_path, output_datatype, input_datatype
            )
        else:
    ...
    def get_embedding(
        ...
        with torch.no_grad():
            ...
            sentences_embed = self.mean_pooling(
                model_output, encode_inputs["attention_mask"].to(self.device)
            )
        ...
        if type_input == "txt":
            ...
            for cxx_context_path in context_paths:
                ...
                for path in paths:
                    with open(path) as f:
                        data = [
                            "nan" if (x == "" or x == []) else x for x in json.load(f)
                        ]
                        raw_data += data
        else:
